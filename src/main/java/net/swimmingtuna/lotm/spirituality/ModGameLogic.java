...
public class ModGameLogic {
    public static void addSpirituality(Entity pEntity) {
        if (pEntity instanceof Player) {
            ...
            pEntity.getCapability(TyrantSequenceProvider.TYRANTSEQUENCE).ifPresent(tyrantSequence -> {
            pEntity.getCapability(SpectatorSequenceProvider.SPECTATORSEQUENCE).ifPresent(spectatorSequence -> {
                if (spiritualityAttribute != null && spectatorSequence.getSpectatorSequence() >= 1  || tyrantSequence.getTyrantSequence() >= 1) {
